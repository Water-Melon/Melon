...
{
    if (lm == NULL) return;
    if (lm->retExp != NULL) mln_lang_retExp_free(lm->retExp);
...
static void mln_lang_mysql_connect_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status == NET_ASYNC_ERROR) {
        ...
        if ((retExp = mln_lang_retExp_createTmpTrue(ctx->pool, NULL)) != NULL) {
            ...
            val = var->val;
            mln_lang_ctx_setRetExp(mysql->ctx, retExp);
...
static mln_lang_retExp_t *mln_lang_mysql_connect_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
...
static int mln_lang_mysql_add_connect(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_connect_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    for (i = 0; i < sizeof(arr)/sizeof(mln_string_t); ++i) {
        ...
        if ((var = mln_lang_var_new(ctx->pool, &arr[i], M_LANG_VAR_NORMAL, val, NULL)) == NULL) {
            mln_lang_errmsg(ctx, "No memory.");
            mln_lang_val_free(val);
            mln_lang_func_detail_free(func);
            return -1;
...
static mln_lang_retExp_t *mln_lang_mysql_close_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_close(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_close_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_FUNC, func)) == NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_commit_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_commit(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_commit_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_FUNC, func)) == NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_rollback_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_rollback(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_rollback_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_FUNC, func)) == NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_error_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_error(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_error_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_FUNC, func)) == NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_errno_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_errno(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_errno_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_FUNC, func)) == NULL) {
...
static int mln_lang_mysql_build_return_value(mln_lang_mysql_t *mysql)
{
    ...
    mln_size_t *length;
    mln_lang_ctx_t *ctx = mysql->ctx;
    if (mysql->retExp == NULL) {
        if ((mysql->retExp = mln_lang_retExp_createTmpArray(ctx, NULL)) == NULL) {
            mln_lang_errmsg(ctx, "No memory.");
            return -1;
        }
    }
    array = mysql->retExp->data.var->val->data.array;
...
static void mln_lang_mysql_fetch_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status != NET_ASYNC_COMPLETE) {
        ...
        if (mysql->row) {
            ...
            if (mysql->retExp == NULL && (mysql->retExp = mln_lang_retExp_createTmpArray(ctx, NULL)) == NULL) {
                mln_lang_errmsg(ctx, "No memory.");
            }
            if (mysql->retExp != NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_execute_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
...
static int mln_lang_mysql_add_execute(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_execute_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
...
static mln_lang_retExp_t *mln_lang_mysql_autocommit_process(mln_lang_ctx_t *ctx)
{
    mln_s32_t type;
    mln_lang_val_t *val1, *val;
    mln_string_t _this = mln_string("this");
...
static int mln_lang_mysql_add_autocommit(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *set)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_mysql_autocommit_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
...
int mln_lang_mysql(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_lang_resource_register(ctx, "mysql", lmt, (mln_lang_resource_free)mln_lang_mysql_timeout_free) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
