...
static int mln_lang_json_encode_obj_scan(void *rn_data, void *udata);
...
static int mln_lang_json_decode_array_scan(void *rn_data, void *udata);
...
static int mln_lang_json_encode_handler(mln_lang_ctx_t *ctx)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_json_encode_process, NULL)) == NULL) {
...
static int mln_lang_json_decode_handler(mln_lang_ctx_t *ctx)
{
    ...
    if ((func = mln_lang_func_detail_new(ctx->pool, M_FUNC_INTERNAL, mln_lang_json_decode_process, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx->pool, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
...
static int mln_lang_json_decode_obj_scan(void *key, void *val, void *data)
{
    ...
    if (M_JSON_IS_OBJECT(jo->val)) {
        ...
        mln_lang_val_t val;
        if ((tmpa = mln_lang_array_new(ljs->ctx->pool)) == NULL) {
            return -1;
        }
        ...
        val.ref = 1;
        if (mln_lang_var_setValue(ljs->ctx->pool, array_val, &var) < 0) {
            mln_lang_array_free(tmpa);
            return -1;
        }
        mln_lang_array_free(tmpa);
...
mln_lang_json_decode_array(mln_lang_ctx_t *ctx, mln_lang_array_t *array, mln_rbtree_t *a)
{
    ...
    ljs.ctx = ctx;
    ljs.array = array;
    if (mln_rbtree_scan_all(a, a->root, mln_lang_json_decode_array_scan, &ljs) < 0) {
        return -1;
    }
    return 0;
}
