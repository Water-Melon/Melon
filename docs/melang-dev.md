...
本篇将介绍如何开发脚本语言内置库和动态扩展库，同时也会给出开发所需要的各类函数。
...
1. 在`Melon/melang`目录下新建一个目录，目录的名字为函数或者类结构的名字，暂时计作`FUNCNAME`。

2. 在步骤1新建的目录中创建源文件与头文件，命名规则为：`mln_lang_FUNCNAME.c/h`。

3. 在头文件和源文件中必须定义一个名为`mln_lang_FUNCNAME`的函数，函数原型如下：

   ```c
   typedef int (*melang_installer)(mln_lang_ctx_t *);
   ```

4. 在步骤3的函数内初始化类和（或）函数，一般分为两部分：函数名及参数定义 和 函数功能具体实现。
