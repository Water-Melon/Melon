 ...
 */
static int
mln_pg_calc_first_rbtree_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_calc_follow_rbtree_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_closure_rbtree_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static mln_pg_item_t *
mln_pg_rule_duplicate(mln_pg_state_t *s, mln_pg_rule_t *r, mln_u32_t pos);
static int
mln_pg_output_token_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_output_token_set_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_calc_info_cmp(const void *data1, const void *data2);
static int
mln_pg_goto_la_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_goto_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static int
mln_pg_output_state_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata);
static inline int
 ...
 * mln_pg_token_t
 */
mln_pg_token_t *mln_pg_token_new(mln_string_t *token, mln_u32_t nr_rule)
{
...
void mln_pg_token_free(void *token)
{
...
int mln_pg_token_rbtree_cmp(const void *data1, const void *data2)
{
...
void mln_pg_map_hash_free(void *data)
{
    if (data == NULL) return;
    free(data);
}
 ...
 * mln_pg_item_t
 */
mln_pg_item_t *mln_pg_item_new(void)
{
...
void mln_pg_item_free(mln_pg_item_t *item)
{
    if (item == NULL) return;
    if (item->lookahead_set != NULL) {
 ...
 * state
 */
mln_pg_state_t *mln_pg_state_new(void)
{
...
void mln_pg_state_free(mln_pg_state_t *s)
{
    mln_pg_item_t *it;
 ...
 */
int mln_pg_calc_info_init(struct mln_pg_calc_info_s *pci, \
 ...
 * calc nullable
 */
int mln_pg_calc_nullable(volatile int *map, mln_pg_rule_t *r, mln_u32_t nr_rule)
{
 ...
 * calc first
 */
int mln_pg_calc_first(volatile int *map, mln_pg_rule_t *r, mln_u32_t nr_rule)
{
    int i, j, change = 1;
    mln_pg_rule_t *pr;
    mln_pg_token_t *tk;
 ...
 * calc follow
 */
int mln_pg_calc_follow(volatile int *map, mln_pg_rule_t *r, mln_u32_t nr_rule)
{
    int i, j, k, change = 1, nr_null;
    mln_pg_rule_t *pr;
 ...
 * closure
 */
int mln_pg_closure(mln_pg_state_t *s, mln_pg_rule_t *r, mln_u32_t nr_rule)
{
    int i, change = 1;
...
mln_pg_goto_la_iterate_handler(mln_rbtree_node_t *node, void *rn_data, void *udata)
{
    ...
    mln_pg_token_t *tk = (mln_pg_token_t *)rn_data;
    mln_rbtree_node_t *rn;
    rn = mln_rbtree_search(info->item->lookahead_set, \
                           info->item->lookahead_set->root, \
                           tk);
 ...
 * output information
 */
void mln_pg_output_token(mln_rbtree_t *tree, mln_pg_rule_t *r, mln_u32_t nr_rule)
{
    mln_pg_token_t *tk;
    mln_pg_rule_t *pr;
    int i, j;
