...
static int mln_http_hash_calc(mln_hash_t *h, void *key);
...
static int mln_http_dump_scan(void *key, void *val, void *data);
static inline int
...
static int
mln_http_generate_fields_hash_scan(void *key, void *val, void *data);
static inline int
...
int mln_http_parse(mln_http_t *http, mln_chain_t **in)
{
    ...
    mln_size_t len = 0;
    mln_http_handler handler = mln_http_get_handler(http);

    while (!mln_http_get_Done(http) && \
...
static inline int mln_http_line_length(mln_http_t *http, mln_chain_t *in, mln_size_t *len)
{
    ...
    while (in != NULL) {
        ...
        for (p = b->send_pos, end = b->last; p < end; p++) {
            if (*p == (mln_u8_t)'\n') break;
            length++;
        }
        if (p >= end) {
...
static inline int mln_http_process_line(mln_http_t *http, mln_chain_t **in, mln_size_t len)
{
    ...
    mln_u32_t type = mln_http_get_type(http);
...
static inline int mln_http_parse_headline(mln_http_t *http, mln_u8ptr_t buf, mln_size_t len)
{
    ...
    for (scan = http_version; scan < send; scan++) {
        if (!mln_strcasecmp(&tmp, scan)) break;
    }
    if (scan < send) {
        ...
        if (scan < send) {
            ...
            mln_http_set_method(http, scan - http_method);
        } else { 
    ...
    if (type == M_HTTP_REQUEST) {
        ...
        if (ques == NULL || ques+1 >= p) {
            tmp.str = (mln_s8ptr_t)buf;
            tmp.len = (ques == NULL)? p-buf: ques-buf;
            tmp.is_referred = 1;
            s = mln_dup_string_pool(pool, &tmp);
            if (s == NULL) {
                mln_http_set_error(http, M_HTTP_INTERNAL_SERVER_ERROR);
                return M_HTTP_RET_ERROR;
            }
        ...
        mln_http_set_status(http, status);
    }
    buf = p;
    ...
    if (buf >= end) {
        if (type == M_HTTP_REQUEST) {
            ...
            mln_http_set_response_msg(http, NULL);
        }
        return M_HTTP_RET_OK;
    }
    ...
    mln_http_set_response_msg(http, s);
    return M_HTTP_RET_OK;
}
...
static inline int mln_http_parse_field(mln_http_t *http, mln_u8ptr_t buf, mln_size_t len)
{
    ...
    tmp.len = end - buf;
    tmp.is_referred = 1;
    v = mln_dup_string_pool(pool, &tmp);
    if (v == NULL) {
        mln_free_string_pool(s);
        mln_http_set_error(http, M_HTTP_INTERNAL_SERVER_ERROR);
        return M_HTTP_RET_ERROR;
...
int mln_http_generate(mln_http_t *http, mln_chain_t **out_head, mln_chain_t **out_tail)
{
    ...
    if (header_fields != NULL) {
        if (mln_hash_scan_all(header_fields, \
                              mln_http_generate_fields_hash_scan, \
                              &hc) < 0)
            goto err;
    }
...
mln_http_generate_write(struct mln_http_chain_s *hc, void *buf, mln_size_t size)
{
    ...
    while (size > 0) {
        if (hc->left_size == 0) {
            ...
            b->send_pos = b->pos = b->start = buffer;
            b->last = b->end = buffer;
            b->in_memory = 1;
            b->last_buf = 1;
...
mln_http_generate_version(struct mln_http_chain_s *hc)
{
    mln_u32_t version = mln_http_get_version(hc->http);

    if (version >= sizeof(http_version)/sizeof(mln_string_t)) {
        if (mln_http_get_type(hc->http) == M_HTTP_REQUEST)
            mln_http_set_error(hc->http, M_HTTP_BAD_REQUEST);
        else
            mln_http_set_error(hc->http, M_HTTP_UNPARSEABLE_RESPONSE_HEADERS);
        return M_HTTP_RET_ERROR;
    }

    mln_string_t *p = &http_version[version];
    if (mln_http_generate_write(hc, p->str, p->len) == M_HTTP_RET_ERROR) {
        return M_HTTP_RET_ERROR;
    }
    return M_HTTP_RET_OK;
}
...
mln_http_generate_status(struct mln_http_chain_s *hc)
{
    mln_u32_t status = mln_http_get_status(hc->http);
    ...
    if (mln_http_generate_write(hc, map->code_str.str, map->code_str.len) == M_HTTP_RET_ERROR)
        return M_HTTP_RET_ERROR;
    if (mln_http_generate_write(hc, " ", 1) == M_HTTP_RET_ERROR)
        return M_HTTP_RET_ERROR;
    if (mln_http_generate_write(hc, map->msg_str.str, map->msg_str.len) == M_HTTP_RET_ERROR)
        return M_HTTP_RET_ERROR;
...
mln_http_generate_method(struct mln_http_chain_s *hc)
{
    mln_u32_t method = mln_http_get_method(hc->http);
...
mln_http_generate_uri(struct mln_http_chain_s *hc)
{
    mln_string_t *uri = mln_http_get_uri(hc->http);
    if (uri == NULL) {
        if (mln_http_generate_write(hc, "/", 1) == M_HTTP_RET_ERROR)
            return M_HTTP_RET_ERROR;
    } else {
        if (mln_http_generate_write(hc, uri->str, uri->len) == M_HTTP_RET_ERROR)
            return M_HTTP_RET_ERROR;
    }

    mln_string_t *args = mln_http_get_args(hc->http);
    if (args != NULL) {
        if (mln_http_generate_write(hc, "?", 1) == M_HTTP_RET_ERROR)
            return M_HTTP_RET_ERROR;
        if (mln_http_generate_write(hc, args->str, args->len) == M_HTTP_RET_ERROR)
            return M_HTTP_RET_ERROR;
    }
...
mln_http_generate_fields_hash_scan(void *key, void *val, void *data)
{
    ...
    if (mln_http_generate_write(hc, k->str, k->len) == M_HTTP_RET_ERROR)
        return -1;
    if (mln_http_generate_write(hc, ": ", 2) == M_HTTP_RET_ERROR)
        return -1;
    if (val != NULL) {
        if (mln_http_generate_write(hc, v->str, v->len) == M_HTTP_RET_ERROR)
            return -1;
    }
    if (mln_http_generate_write(hc, "\r\n", 2) == M_HTTP_RET_ERROR)
        return -1;
...
int mln_http_set_field(mln_http_t *http, mln_string_t *key, mln_string_t *val)
{
    ...
    dup_key = mln_dup_string_pool(pool, key);
    if (dup_key == NULL) return M_HTTP_RET_ERROR;
    dup_val = mln_dup_string_pool(pool, val);
    if (dup_val == NULL) {
        mln_free_string_pool(dup_key);
        return M_HTTP_RET_ERROR;
    }
    int ret = mln_hash_replace(header_fields, &dup_key, &dup_val);
    mln_free_string_pool(dup_key);
    mln_free_string_pool(dup_val);
...
void mln_http_destroy(mln_http_t *http)
{
    ...
    if (http->uri != NULL) {
        mln_free_string_pool(http->uri);
    }
    if (http->args != NULL) {
        mln_free_string_pool(http->args);
    }
    if (http->response_msg != NULL) {
        mln_free_string_pool(http->response_msg);
    }
...
static void mln_http_hash_free(void *data)
{
    mln_free_string_pool((mln_string_t *)data);
}
...
void mln_http_dump(mln_http_t *http)
{
    ...
    if (http->uri != NULL) {
        ...
        write(STDOUT_FILENO, http->uri->str, http->uri->len);
        printf("]\n");
    }
    if (http->args != NULL) {
        printf("\tARGS:[");fflush(stdout);
        write(STDOUT_FILENO, http->args->str, http->args->len);
        printf("]\n");
    }
    if (http->response_msg != NULL) {
        printf("\tRESPONSE_MSG:[");fflush(stdout);
        write(STDOUT_FILENO, http->response_msg->str, http->response_msg->len);
        printf("]\n");
    }
...
static int mln_http_dump_scan(void *key, void *val, void *data)
{
    printf("\t\tkey:[%s] value:[%s]\n", ((mln_string_t *)key)->str, val == NULL?"NULL":((mln_string_t *)val)->str);
