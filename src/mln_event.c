...
static inline int
mln_event_deal_timer(mln_event_t *event) __NONNULL1(1);
static inline int
mln_event_set_fd_normal(mln_event_t *event, \
                        ...
                        mln_u32_t flag, \
                        void *data, \
                        void (*norm_handler)(mln_event_t *, int, void *), \
                        int other_mark);
static inline void
mln_event_set_fd_append(mln_event_t *event, \
                        ...
                        mln_u32_t flag, \
                        void *data, \
                        void (*fd_handler)(mln_event_t *, int, void *), \
                        int other_mark);
static inline int
mln_event_set_signal_set(mln_event_t *event, \
                         int signo, \
                         void *data, \
                         void (*set_handler)(mln_event_t *, int, void *));
static inline int
mln_event_set_signal_unset(mln_event_t *event, \
                           int signo, \
                           void *data, \
                           void (*unset_handler)(mln_event_t *, int, void *));
static void
...
mln_event_t *mln_event_init(mln_u32_t is_main)
{
    ...
    FD_ZERO(&(ev->err_set));
#endif
    if (mln_event_set_fd(ev, ev->rd_fd, M_EV_RECV, NULL, mln_event_signal_fd_handler) < 0)
        goto err1;
    if (is_main) {
...
lp:
    ...
    rn = mln_rbtree_search(ev->ev_signal_tree, \
                           ...
                           &esc);
    if (rn == &(ev->ev_signal_tree->nil)) return;
...
int mln_event_set_signal(mln_event_t *event, \
                         ...
                         int signo, \
                         void *data, \
                         void (*sg_handler)(mln_event_t *, int, void *))
{
    int ret;
    switch (flag) {
        case M_EV_SET:
                         ...
                         void (*set_handler)(mln_event_t *, int, void *))
{
    ...
    ed->data.sig.handler = set_handler;
    ...
    rn = mln_rbtree_search(event->ev_signal_tree, \
                           ...
                           &esc);
    if (rn == &(event->ev_signal_tree->nil)) {
    ...
    rn = mln_rbtree_search(ev_signal_refer_tree, \
                           ...
                           &esr);
    if (rn == &(ev_signal_refer_tree->nil)) {
                           ...
                           void (*unset_handler)(mln_event_t *, int, void *))
{
    ...
    for (ed = escp->sig_head; ed != NULL; ed = ed->next) {
        if (ed->data.sig.data == data && \
            ed->data.sig.handler == unset_handler)
        break;
    }
    if (ed == NULL) {
...
int mln_event_set_timer(mln_event_t *event, \
                        mln_u32_t msec, \
                        void *data, \
                        void (*tm_handler)(mln_event_t *, void *))
{
    struct timeval tv;
...
static inline int
mln_event_deal_timer(mln_event_t *event)
{
    mln_uauto_t now;
    struct timeval tv;
    gettimeofday(&tv, NULL);
                     ...
                     void (*fd_handler)(mln_event_t *, int, void *))
{
    ...
    if (rn != &(event->ev_fd_tree->nil)) {
        if (flag & M_EV_APPEND) {
            ...
            mln_event_set_fd_normal(event, \
                                    (mln_event_desc_t *)(rn->data), \
                                    fd, \
                                    flag, \
                                    data, \
                                    fd_handler, \
                                    ((mln_event_desc_t *)(rn->data))->data.fd.is_clear?0:1);
        }
        return 0;
    }
    if (flag & M_EV_NONBLOCK) {
    ...
    if (mln_event_set_fd_normal(event, NULL, fd, flag, data, fd_handler, 0) < 0)
        return -1;
    return 0;
}
                        ...
                        int other_mark)
{
    if (ed == NULL) {
        ...
        if (ed->data.fd.is_clear) {
            ed->data.fd.rcv_data = NULL;
            ed->data.fd.rcv_handler = NULL;
            ed->data.fd.snd_data = NULL;
            ed->data.fd.snd_handler = NULL;
            ed->data.fd.err_data = NULL;
            ed->data.fd.err_handler = NULL;
            ed->data.fd.active_flag = 0;
            ed->data.fd.in_process = 0;
	    ed->data.fd.is_clear = 0;
    ...
    mln_event_set_fd_append(event, ed, fd, flag, data, norm_handler, other_mark);
    return 0;
}
...
mln_event_set_fd_append(mln_event_t *event, \
                        ...
                        mln_u32_t flag, \
                        void *data, \
                        void (*app_handler)(mln_event_t *, int, void *), \
                        int other_mark)
{
#if defined(MLN_EPOLL)
#define CASE_MACRO(flg); \
    ...
    if (flag & M_EV_RECV) {
        ...
        ed->data.fd.rcv_handler = app_handler;
    ...
    if (flag & M_EV_SEND) {
        ...
        ed->data.fd.snd_handler = app_handler;
    ...
    if (flag & M_EV_ERROR) {
        ...
        ed->data.fd.err_handler = app_handler;
...
void mln_dispatch(mln_event_t *event)
{
    ...
    while (1) {
        ret = mln_event_deal_timer(event);
        mln_event_deal_active_fd(event);
        if (event->is_child_proc) {
            mln_event_destroy(event);
            return;
        }
        if (event->is_break) {
            event->is_break = 0;
...
mln_event_deal_active_fd(mln_event_t *event)
{
    ...
    while ((ed = event->ev_fd_active_head) != NULL) {
        ...
        ef->in_active = 0;
        ef->in_process = 1;
        if (ef->is_clear) ef->active_flag = 0;
...
mln_event_sig_decrease_refer(int signo)
{
    ...
    rn  = mln_rbtree_search(ev_signal_refer_tree, \
                            ev_signal_refer_tree->root, \
                            &esr);
    if (rn == &(ev_signal_refer_tree->nil)) return;
