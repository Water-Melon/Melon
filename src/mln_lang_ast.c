...
MLN_DEFINE_PARSER_GENERATOR(static, \
                            ...
                            {LANG_TK_NEW, "LANG_TK_NEW"}, \
                            {LANG_TK_FREE, "LANG_TK_FREE"}, \
...
static inline mln_lang_class_t *
...
mln_lang_switch_new(mln_alloc_t *pool, mln_lang_exp_t *exp, mln_lang_switchstm_t *switchstm);
...
mln_lang_while_new(mln_alloc_t *pool, mln_lang_exp_t *exp, mln_lang_block_t *blockstm);
...
mln_lang_suffix_new(mln_alloc_t *pool, mln_lang_locate_t *left, mln_lang_suffix_op_t op);
...
mln_lang_funccall_new(mln_alloc_t *pool, mln_string_t *name, mln_lang_exp_t *args);
static void mln_lang_funccall_free(void *data);
static inline mln_lang_factor_t *
mln_lang_factor_new(mln_alloc_t *pool, mln_lang_factor_type_t type, void *data);
...
static int mln_lang_semantic_speclocate(mln_factor_t *left, mln_factor_t **right, void *data);
...
{"__locate_exp: LANG_TK_PERIOD LANG_TK_ID", mln_lang_semantic_locateproperty},
{"__locate_exp: ", NULL},
...
{"spec_exp: LANG_TK_NEW LANG_TK_ID LANG_TK_LPAR exp LANG_TK_RPAR", mln_lang_semantic_specnew},
{"spec_exp: LANG_TK_FREE spec_exp", mln_lang_semantic_specfree},
                 ...
                 mln_lang_stm_t *next)
{
    ...
    switch (type) {
        ...
        case M_STM_FUNC:
            ...
            break;
        case M_STM_CLASS:
            ls->data.classdef = (mln_lang_class_t *)data;
            break;
        case M_STM_LABEL:
    ...
    while (next != NULL) {
        ...
        switch (type) {
            ...
            case M_STM_FUNC:
                ...
                break;
            case M_STM_CLASS:
                if (stm->data.classdef != NULL) mln_lang_class_free(stm->data.classdef);
                break;
            case M_STM_LABEL:
...
static int mln_lang_semantic_stmBlock(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_lang_stm_t *stm;
    if ((stm = mln_lang_stm_new(pool, right[0]->data, M_STM_BLOCK, (mln_lang_stm_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = stm;
...
static int mln_lang_semantic_stmclass(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((stm =  mln_lang_stm_new(pool, clas, M_STM_CLASS, (mln_lang_stm_t *)(right[6]->data))) == NULL) {
        mln_lang_class_free(clas);
        return -1;
    }
    right[2]->data = NULL;
    right[4]->data = NULL;
...
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_block_t *lb = mln_lang_block_new(pool, right[0]->data, M_BLOCK_EXP);
    if (lb == NULL) return -1;
    left->data = lb;
...
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_block_t *lb = mln_lang_block_new(pool, right[1]->data, M_BLOCK_STM);
    if (lb == NULL) return -1;
    left->data = lb;
    left->nonterm_free_handler = mln_lang_block_free;
    right[1]->data = NULL;
...
static int mln_lang_semantic_labelstm(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_lang_stm_t *stm = mln_lang_stm_new(pool, label, M_STM_LABEL, (mln_lang_stm_t *)(right[2]->data));
    if (stm == NULL) {
...
static int mln_lang_semantic_whilestm(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (w == NULL) return -1;
    mln_lang_stm_t *stm = mln_lang_stm_new(pool, w, M_STM_WHILE, (mln_lang_stm_t *)(right[5]->data));
    if (stm == NULL) {
...
static int mln_lang_semantic_forstm(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (f == NULL) return -1;
    mln_lang_stm_t *stm = mln_lang_stm_new(pool, f, M_STM_FOR, (mln_lang_stm_t *)(right[9]->data));
    if (stm == NULL) {
        mln_lang_for_free(f);
        return -1;
    }
...
static int mln_lang_semantic_ifstm(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (i == NULL) return -1;
    mln_lang_stm_t *stm = mln_lang_stm_new(pool, i, M_STM_IF, (mln_lang_stm_t *)(right[6]->data));
    if (stm == NULL) {
        mln_lang_if_free(i);
        return -1;
    }
...
static int mln_lang_semantic_switchstm(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (sw == NULL) return -1;
    mln_lang_stm_t *stm = mln_lang_stm_new(pool, sw, M_STM_SWITCH, (mln_lang_stm_t *)(right[7]->data));
    if (stm == NULL) {
...
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_block_t *block = mln_lang_block_new(pool, NULL, M_BLOCK_CONTINUE);
...
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_block_t *block = mln_lang_block_new(pool, NULL, M_BLOCK_BREAK);
    if (block == NULL) return -1;
    left->data = block;
    left->nonterm_free_handler = mln_lang_block_free;
    return 0;
...
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_block_t *block = mln_lang_block_new(pool, right[1]->data, M_BLOCK_RETURN);
    if (block == NULL) return -1;
    left->data = block;
    left->nonterm_free_handler = mln_lang_block_free;
    right[1]->data = NULL;
...
static int mln_lang_semantic_goto(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_lang_block_t *block = mln_lang_block_new(pool, pos, M_BLOCK_GOTO);
    if (block == NULL) {
...
static int mln_lang_semantic_assignexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((assign = mln_lang_assign_new(pool, (mln_lang_logicLow_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = assign;
...
static int mln_lang_semantic_logicLowexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((ll = mln_lang_logicLow_new(pool, (mln_lang_logicHigh_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = ll;
...
static int mln_lang_semantic_logicHighexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((ll = mln_lang_logicHigh_new(pool, (mln_lang_relativeLow_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = ll;
...
static int mln_lang_semantic_relativeLowexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((lr = mln_lang_relativeLow_new(pool, (mln_lang_relativeHigh_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = lr;
...
static int mln_lang_semantic_relativeHighexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((lr = mln_lang_relativeHigh_new(pool, (mln_lang_move_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = lr;
...
static int mln_lang_semantic_moveexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((move = mln_lang_move_new(pool, (mln_lang_addsub_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = move;
...
static int mln_lang_semantic_addsubexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((la = mln_lang_addsub_new(pool, (mln_lang_muldiv_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = la;
...
static int mln_lang_semantic_muldivexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((lm = mln_lang_muldiv_new(pool, (mln_lang_suffix_t *)(right[0]->data), op, r)) == NULL) {
        return -1;
    }
    left->data = lm;
...
static int mln_lang_semantic_suffixexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((ls = mln_lang_suffix_new(pool, (mln_lang_locate_t *)(right[0]->data), op)) == NULL) {
        return -1;
    }
    left->data = ls;
...
static int mln_lang_semantic_locateexp(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    switch (op) {
        case M_LOCATE_INDEX:
            if ((ll = mln_lang_locate_new(pool, (mln_lang_spec_t *)(right[0]->data), op, tmp->locate.assign)) == NULL)
            {
                return -1;
            }
            ...
            break;
        case M_LOCATE_PROPERTY:
            if ((ll = mln_lang_locate_new(pool, (mln_lang_spec_t *)(right[0]->data), op, tmp->locate.id)) == NULL)
            {
                return -1;
            }
            tmp->locate.id = NULL;
            break;
        default:
            if ((ll = mln_lang_locate_new(pool, (mln_lang_spec_t *)(right[0]->data), op, NULL)) == NULL)
            {
                return -1;
            }
            break;
...
static int mln_lang_semantic_locateproperty(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (id == NULL) return -1;
    if ((tmp = mln_lang_locate_tmp_new(pool, M_LOCATE_PROPERTY, id)) == NULL) {
...
static int mln_lang_semantic_specnegative(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_NEGATIVE, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
...
static int mln_lang_semantic_specreverse(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_REVERSE, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specnot(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_NOT, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specrefer(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_REFER, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_speclocate(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_LOCATE, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specinc(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_INC, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specdec(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_DEC, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfree(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_FREE, (mln_lang_spec_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specnew(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_string_t *name = mln_string_pool_dup(pool, ls->text);
    if (name == NULL) return -1;
    mln_lang_funccall_t *func = mln_lang_funccall_new(pool, name, (mln_lang_exp_t *)(right[3]->data));
    if (func == NULL) {
        mln_string_pool_free(name);
        return -1;
    }
    if ((spec = mln_lang_spec_new(pool, M_SPEC_NEW, func)) == NULL) {
...
static int mln_lang_semantic_specparenth(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_PARENTH, (mln_lang_assign_t *)(right[1]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfactor(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_FACTOR, (mln_lang_factor_t *)(right[0]->data))) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfunc(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_string_t *name = mln_string_pool_dup(pool, ls->text);
    if (name == NULL) return -1;
    mln_lang_funccall_t *func = mln_lang_funccall_new(pool, name, (mln_lang_exp_t *)(right[2]->data));
    if (func == NULL) {
        mln_string_pool_free(name);
        return -1;
    }
    if ((spec = mln_lang_spec_new(pool, M_SPEC_FUNC, func)) == NULL) {
        mln_lang_funccall_free(func);
        return -1;
    }
    left->data = spec;
...
static int mln_lang_semantic_factortrue(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    mln_lang_factor_t *lf;
    mln_u8_t t = 1;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_BOOL, &t)) == NULL) {
        return -1;
    }
    left->data = lf;
...
static int mln_lang_semantic_factornil(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_NIL, NULL)) == NULL) {
        return -1;
    }
    left->data = lf;
    left->nonterm_free_handler = mln_lang_factor_free;
...
static int mln_lang_semantic_factorid(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (id == NULL) return -1;
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_ID, id)) == NULL) {
        mln_string_pool_free(id);
        return -1;
    }
    left->data = lf;
...
#endif
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_INT, &i)) == NULL) {
        return -1;
    }
    left->data = lf;
    left->nonterm_free_handler = mln_lang_factor_free;
...
static int mln_lang_semantic_factorreal(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    double f = atof(num);
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_REAL, &f)) == NULL) {
        return -1;
    }
    left->data = lf;
    left->nonterm_free_handler = mln_lang_factor_free;
...
static int mln_lang_semantic_factorstring(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (s == NULL) return -1;
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_STRING, s)) == NULL) {
...
static int mln_lang_semantic_factorarray(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_factor_t *lf;
    if ((lf = mln_lang_factor_new(pool, M_FACTOR_REAL, right[1]->data)) == NULL) {
        return -1;
    }
    left->data = lf;
    left->nonterm_free_handler = mln_lang_factor_free;
...
static int mln_lang_semantic_elemlist(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((le = mln_lang_elemlist_new(pool, \
                                    ...
                                    (mln_lang_elemlist_t *)(right[2]->data))) == NULL)
    {
        return -1;
    }
    left->data = le;
