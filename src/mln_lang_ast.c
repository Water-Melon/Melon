...
mln_lang_classstm_new(mln_alloc_t *pool, \
                      void *data, \
...
static void mln_lang_property_free(void *data);
...
static int mln_lang_semantic_propertyfunc(mln_factor_t *left, mln_factor_t **right, void *data);
                    ...
                    mln_u64_t line)
{
    ...
    switch (op) {
        ...
        case M_LOCATE_PROPERTY:
            ll->right.property = (mln_lang_property_t *)right;
            break;
        default:
...
static void mln_lang_locate_free(void *data)
{
    ...
    switch (op) {
        case M_LOCATE_INDEX:
            ...
            break;
        case M_LOCATE_PROPERTY:
            if (ll->right.property != NULL) mln_lang_property_free(ll->right.property);
            break;
        default:
            break;
    }
                        ...
                        void *data)
{
    ...
    switch (op) {
        case M_LOCATE_INDEX:
            ...
            break;
        case M_LOCATE_PROPERTY:
            llt->locate.property = (mln_lang_property_t *)data;
            break;
        default:
...
static void mln_lang_locate_tmp_free(void *data)
{
    ...
    switch (op) {
        case M_LOCATE_INDEX:
            ...
            break;
        case M_LOCATE_PROPERTY:
            if (llt->locate.property != NULL) mln_lang_property_free(llt->locate.property);
            break;
        default:
            break;
    }
...
mln_lang_property_new(mln_alloc_t *pool, mln_lang_property_type_t type, void *data, mln_u64_t line)
{
    mln_lang_property_t *lp;
    if ((lp = (mln_lang_property_t *)mln_alloc_m(pool, sizeof(mln_lang_property_t))) == NULL) {
        return NULL;
    }
    lp->line = line;
    lp->type = type;
    switch (type) {
        case M_PROPERTY_NOARGS:
            lp->data.id = (mln_string_t *)data;
            break;
        default:
            lp->data.func = (mln_lang_funccall_t *)data;
            break;
    }
    return lp;
}
...
{
    if (data == NULL) return;
    mln_lang_property_t *lp = (mln_lang_property_t *)data;
    switch (lp->type) {
        case M_PROPERTY_NOARGS:
            if (lp->data.id != NULL) mln_string_pool_free(lp->data.id);
            break;
        default:
            if (lp->data.func != NULL) mln_lang_funccall_free(lp->data.func);
            break;
    }
    mln_alloc_free(lp);
}
...
static int mln_lang_semantic_stmclass(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if ((clas = mln_lang_class_new(pool, \
                                   ...
                                   (mln_lang_classstm_t *)(right[4]->data), \
                                   left->line)) == NULL)
    {
...
static int mln_lang_semantic_locateproperty(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (id == NULL) return -1;
    if (right[2]->data == NULL) {
        if ((property = mln_lang_property_new(pool, M_PROPERTY_NOARGS, id, left->line)) == NULL) {
            mln_string_pool_free(id);
            return -1;
        }
    } else {
        mln_lang_funccall_t *func;
        if ((func = mln_lang_funccall_new(pool, id, right[2]->data, left->line)) == NULL) {
            mln_string_pool_free(id);
            return -1;
        }
        if ((property = mln_lang_property_new(pool, M_PROPERTY_ARGS, func, left->line)) == NULL) {
...
static int mln_lang_semantic_specnegative(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_NEGATIVE, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
...
static int mln_lang_semantic_specreverse(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_REVERSE, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specnot(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_NOT, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specrefer(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_REFER, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_speclocate(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_LOCATE, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specinc(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_INC, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specdec(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_DEC, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfree(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_FREE, (mln_lang_spec_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specparenth(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_PARENTH, (mln_lang_assign_t *)(right[1]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfactor(mln_factor_t *left, mln_factor_t **right, void *data)
{
    mln_alloc_t *pool = (mln_alloc_t *)data;
    mln_lang_spec_t *ls;
    if ((ls = mln_lang_spec_new(pool, M_SPEC_FACTOR, (mln_lang_factor_t *)(right[0]->data), left->line)) == NULL) {
        return -1;
    }
    left->data = ls;
    left->nonterm_free_handler = mln_lang_spec_free;
...
static int mln_lang_semantic_specfunc(mln_factor_t *left, mln_factor_t **right, void *data)
{
    ...
    if (name == NULL) return -1;
    mln_lang_funccall_t *func;
    if ((func = mln_lang_funccall_new(pool, name, (mln_lang_exp_t *)(right[2]->data), left->line)) == \
        NULL)
    {
        mln_string_pool_free(name);
        return -1;
