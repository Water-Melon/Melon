...
__mln_lang_var_createTmpNil(mln_lang_ctx_t *ctx, mln_string_t *name);
static inline mln_lang_var_t *
__mln_lang_var_createTmpCall(mln_lang_ctx_t *ctx, mln_lang_funccall_val_t *call);
static inline mln_lang_var_t *
...
#define mln_lang_sym_scope_chain_add(_head, _tail, _node); \
        (_node)->scope_prev = (_node)->scope_next = NULL;\
        if ((_head) == NULL) {\
            (_head) = (_tail) = (_node);\
        } else {\
            (_tail)->scope_next = (_node);\
            (_node)->scope_prev = (_tail);\
            (_tail) = (_node);\
        }

#define mln_lang_sym_scope_chain_del(_head, _tail, _node) \
        if ((_head) == (_node)) {\
            if ((_tail) == (_node)) {\
                (_head) = (_tail) = NULL;\
            } else {\
                (_head) = (_node)->scope_next;\
                (_head)->scope_prev = NULL;\
            }\
        } else {\
            if ((_tail) == (_node)) {\
                (_tail) = (_node)->scope_prev;\
                (_tail)->scope_next = NULL;\
            } else {\
                (_node)->scope_prev->scope_next = (_node)->scope_next;\
                (_node)->scope_next->scope_prev = (_node)->scope_prev;\
            }\
        }\
        (_node)->scope_prev = (_node)->scope_next = NULL;

#define mln_lang_sym_chain_add(_head, _tail, _node); \
...
#define mln_lang_sym_chain_del(_head, _tail, _node) \
        if ((_head) == (_node)) {\
            if ((_tail) == (_node)) {\
                (_head) = (_tail) = NULL;\
            } else {\
...
#define mln_lang_ctx_reset_ret_var(ctx) \
({\
    if ((ctx)->ret_var != NULL) {\
...
#define mln_lang_ctx_get_node_ret_val(ctx,node) \
({\
    if ((ctx)->ret_var != NULL) {\
        __mln_lang_var_free((ctx)->ret_var);\
    }\
...
#define __mln_lang_ctx_set_ret_var(ctx,var) \
({\
...
#define mln_lang_symbolNode_new(_ctx,_symbol,_type,_data) \
({\
    mln_lang_symbolNode_t *ls;\
    if ((ls = (_ctx)->sym_head) != NULL) {\
        mln_lang_sym_chain_del((_ctx)->sym_head, (_ctx)->sym_tail, ls);\
        --((_ctx)->sym_count);\
    } else {\
        if ((ls = (mln_lang_symbolNode_t *)mln_alloc_m((_ctx)->pool, sizeof(mln_lang_symbolNode_t))) != NULL) {\
            ls->ctx = (_ctx);\
    ...
    if (ls != NULL) {\
        ...
        switch ((_type)) {\
            case M_LANG_SYMBOL_VAR:\
                ls->data.var = (mln_lang_var_t *)_data;\
                break;\
            default: /*M_LANG_SYMBOL_SET*/\
                ls->data.set = (mln_lang_set_detail_t *)_data;\
                break;\
        }\
        ls->layer = (_ctx)->scope_tail->layer;\
        ls->bucket = NULL;\
...
#define mln_lang_symbolNode_free(_data) \
({\
    if ((_data) != NULL) {\
        mln_lang_symbolNode_t *ls = (mln_lang_symbolNode_t *)(_data);\
        switch (ls->type) {\
            case M_LANG_SYMBOL_VAR:\
                if (ls->data.var != NULL) {\
...
#define __mln_lang_symbolNode_join(_ctx,type,_data) \
({\
    mln_lang_symbolNode_t *symbol, *tmp;\
    int rv = 0;\
    mln_string_t *name = (type) == M_LANG_SYMBOL_VAR? ((mln_lang_var_t *)(_data))->name: ((mln_lang_set_detail_t *)(_data))->name;\
\
    if ((symbol = mln_lang_symbolNode_new((_ctx), name, (type), (_data))) == NULL) {\
        __mln_lang_errmsg((_ctx), "No memory.");\
        rv = -1;\
    } else {\
        symbol->bucket = mln_lang_hash_get_bucket((_ctx)->symbols, symbol);\
        for (tmp = symbol->bucket->tail; tmp != NULL; tmp = tmp->prev) {\
             ...
             if (!memcmp(tmp->symbol->data, name->data, name->len)) {\
                 ...
                 mln_lang_sym_scope_chain_del((_ctx)->scope_tail->sym_head, (_ctx)->scope_tail->sym_tail, tmp);\
                 mln_lang_symbolNode_free(tmp);\
                 break;\
             }\
        }\
        mln_lang_sym_chain_add(symbol->bucket->head, symbol->bucket->tail, symbol);\
        mln_lang_sym_scope_chain_add((_ctx)->scope_tail->sym_head, (_ctx)->scope_tail->sym_tail, symbol);\
    }\
    rv;\
})
...
#define __mln_lang_symbolNode_search(ctx,name,_local) \
({\
    int local = (_local);\
    mln_lang_symbolNode_t *sym = NULL, tmp;\
...
\
        for (sym = hb->tail; sym != NULL; sym = sym->prev) {\
            ...
            if (!memcmp(sym->symbol->data, (name)->data, (name)->len)) {\
                local = 1;\
                break;\
            }\
        }\
...
#define mln_lang_symbolNode_idSearch(ctx,name) \
({\
    mln_lang_symbolNode_t *ret = NULL;\
    mln_string_t n = *(name);\
    if ((name)->len > 1 && (name)->data[0] == '_') {\
        n.data += 1;\
        n.len -= 1;\
        ret = __mln_lang_symbolNode_search((ctx), &n, 0);\
    }\
    if (ret == NULL) \
        ret = __mln_lang_symbolNode_search((ctx), (name), 1);\
    ret;\
})
...
mln_lang_var_t *mln_lang_var_createTmpCall(mln_lang_ctx_t *ctx, mln_lang_funccall_val_t *call)
{
    return __mln_lang_var_createTmpCall(ctx, call);
}
...
mln_lang_var_t *mln_lang_var_createTmpNil(mln_lang_ctx_t *ctx, mln_string_t *name)
{
    return __mln_lang_var_createTmpNil(ctx, name);
}
...
mln_lang_var_t *mln_lang_var_createTmpObj(mln_lang_ctx_t *ctx, mln_lang_set_detail_t *inSet, mln_string_t *name)
{
    return __mln_lang_var_createTmpObj(ctx, inSet, name);
}
...
mln_lang_var_t *mln_lang_var_createTmpTrue(mln_lang_ctx_t *ctx, mln_string_t *name)
{
    return __mln_lang_var_createTmpTrue(ctx, name);
}
...
mln_lang_var_t *mln_lang_var_createTmpFalse(mln_lang_ctx_t *ctx, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
    mln_u8_t t = 0;
...
mln_lang_var_t *mln_lang_var_createTmpInt(mln_lang_ctx_t *ctx, mln_s64_t off, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
...
mln_lang_var_t *mln_lang_var_createTmpReal(mln_lang_ctx_t *ctx, double f, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
...
mln_lang_var_t *mln_lang_var_createTmpBool(mln_lang_ctx_t *ctx, mln_u8_t b, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
...
mln_lang_var_t *mln_lang_var_createTmpString_ref(mln_lang_ctx_t *ctx, mln_string_t *s, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
    mln_string_t *dup;
    dup = mln_string_ref_dup(s);
...
mln_lang_var_t *mln_lang_var_createTmpString(mln_lang_ctx_t *ctx, mln_string_t *s, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
    mln_string_t *dup;
...
mln_lang_var_t *mln_lang_var_createTmpArray(mln_lang_ctx_t *ctx, mln_string_t *name)
{
    mln_lang_val_t *val;
    mln_lang_var_t *var;
...
__mln_lang_var_convert(mln_lang_ctx_t *ctx, mln_lang_var_t *var)
{
    ...
    ret->name = var->name != NULL? mln_string_ref_dup(var->name): NULL;
    ret->val = var->val;
    ++(ret->val->ref);
    ret->inSet = var->inSet;
...
__mln_lang_var_dup(mln_lang_ctx_t *ctx, mln_lang_var_t *var)
{
    ...
    if ((ret = __mln_lang_var_new_ref_string(ctx, var->name, var->type, val, var->inSet)) == NULL) {
        if (var->type == M_LANG_VAR_NORMAL && var->val != NULL) {
            __mln_lang_val_free(val);
        }
        return NULL;
...
mln_lang_array_getAndNew_int(mln_lang_ctx_t *ctx, mln_lang_array_t *array, mln_lang_var_t *key)
{
    ...
    mln_lang_array_elem_t *elem;
    if ((nil = __mln_lang_var_createTmpNil(ctx, NULL)) == NULL) {
        __mln_lang_errmsg(ctx, "No memory.");
        return NULL;
    }
...
static void mln_lang_stack_handler_funcdef(mln_lang_ctx_t *ctx)
{
    ...
    if ((var = __mln_lang_var_new_ref_string(ctx, funcdef->name, M_LANG_VAR_NORMAL, val, inSet)) == NULL) {
        __mln_lang_errmsg(ctx, "No memory.");
        __mln_lang_val_free(val);
        mln_lang_job_free(ctx);
        return;
...
again:
        if (ls->type == M_SETSTM_VAR) {
            inSet = mln_lang_ctx_getClass(ctx);
            if ((val = __mln_lang_val_new(ctx, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
                mln_lang_job_free(ctx);
                return;
            }
            if ((var = __mln_lang_var_new_ref_string(ctx, ls->data.var, M_LANG_VAR_NORMAL, val, inSet)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
                __mln_lang_val_free(val);
                mln_lang_job_free(ctx);
                return;
...
static inline int mln_lang_metReturn(mln_lang_ctx_t *ctx)
{
    ...
    if (ctx->ret_var == NULL) {
        ...
        if ((ret_var = __mln_lang_var_createTmpNil(ctx, NULL)) == NULL) {
            __mln_lang_errmsg(ctx, "No memory.");
            return -1;
        }
...
static void mln_lang_stack_handler_while(mln_lang_ctx_t *ctx)
{
    ...
    if (node->step == 0) {
        node->step = 1;
        if (w->condition == NULL) {
            if ((ret_var = __mln_lang_var_createTmpTrue(ctx, NULL)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
                mln_lang_job_free(ctx);
                return;
            }
...
goon1:
        ret_var = ctx->ret_var;
        ASSERT(ret_var->type == M_LANG_RETEXP_VAR);
...
static void mln_lang_stack_handler_switchstm(mln_lang_ctx_t *ctx)
{
    ...
    if (node->step == 0) {
        ...
        method = mln_lang_methods[__mln_lang_var_getValType(sw_node->ret_var)];
        if (method == NULL) {
...
goon2:
        ...
        if (__mln_lang_condition_isTrue(ctx->ret_var)) {
...
goon2:
        ret_var = ctx->ret_var;
        if (ret_var == NULL || __mln_lang_condition_isTrue(ret_var)) {
            node->step = 3;
...
static void mln_lang_stack_handler_assign(mln_lang_ctx_t *ctx)
{
    ...
    if (node->step == 0) {
        ...
        if (assign->op != M_ASSIGN_NONE) {
            ...
            method = mln_lang_methods[__mln_lang_var_getValType(ctx->ret_var)];
            if (method == NULL) {
                __mln_lang_errmsg(ctx, "Operation NOT support.");
                mln_lang_job_free(ctx);
                return;
        ...
        if (handler != NULL) {
            if (mln_lang_val_issetNotModify(node->ret_var->val)) {
...
goon2:
        ...
        mln_lang_op handler = NULL;
        mln_lang_method_t *method;
        method = mln_lang_methods[__mln_lang_var_getValType(node->ret_var)];
        if (method == NULL) {
            __mln_lang_errmsg(ctx, "Operation NOT support.");
            mln_lang_job_free(ctx);
            return;
...
static void mln_lang_stack_handler_suffix(mln_lang_ctx_t *ctx)
{
    ...
    if (node->step == 0) {
        ...
        if (handler != NULL) {
            if (mln_lang_val_issetNotModify(ctx->ret_var->val)) {
                __mln_lang_errmsg(ctx, "Operand cannot be changed.");
                mln_lang_job_free(ctx);
                return;
            }
...
goon3:
        ...
        mln_lang_var_t *res = NULL;
        mln_lang_var_t *var;
        if ((var = mln_lang_var_createTmpString_ref(ctx, locate->right.id, NULL)) == NULL) {
            __mln_lang_errmsg(ctx, "No memory.");
            node->ret_var2 = NULL;
            mln_lang_job_free(ctx);
            return;
...
again_property:
        ...
        mln_lang_var_t *ret_var;
        mln_lang_funccall_val_t *funccall;
        if (mln_lang_var_getValType(ctx->ret_var) != M_LANG_VAL_TYPE_FUNC) {
            mln_string_t *s = NULL;
            if (mln_lang_var_getValType(ctx->ret_var) == M_LANG_VAL_TYPE_STRING) {
                s = mln_lang_var_getVal(ctx->ret_var)->data.s;
            } else if (mln_lang_var_getValType(ctx->ret_var) == M_LANG_VAL_TYPE_NIL && ctx->ret_var->name != NULL) {
                s = ctx->ret_var->name;
            } else {
                __mln_lang_errmsg(ctx, "Operation Not support.");
                mln_lang_job_free(ctx);
            ...
            if (node->ret_var2 != NULL) {
                mln_lang_funccall_val_addObject(funccall, mln_lang_var_getVal(node->ret_var2));
                __mln_lang_var_free(node->ret_var2);
                node->ret_var2 = NULL;
            }
            if ((ret_var = __mln_lang_var_createTmpCall(ctx, funccall)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
        ...
        if (ctx->ret_var != NULL) {
            mln_lang_var_t *var;
            if ((var = __mln_lang_var_convert(ctx, ctx->ret_var)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
                mln_lang_job_free(ctx);
                return;
            }
            mln_lang_funccall_val_addArg(node->ret_var->val->data.call, var);
            mln_lang_ctx_reset_ret_var(ctx);
        }
                                               ...
                                               mln_lang_funccall_val_t *funccall)
{
    ...
    {
        if (var == NULL) {
            if ((newvar = __mln_lang_var_createTmpNil(ctx, scan->name)) == NULL) {
                __mln_lang_errmsg(ctx, "No memory.");
                return -1;
            }
        } else {
...
again:
            ...
            mln_lang_op handler = NULL;
            mln_lang_method_t *method;
            method = mln_lang_methods[__mln_lang_var_getValType(ret_var)];
            if (method == NULL) {
                __mln_lang_errmsg(ctx, "Operation NOT support.");
                mln_lang_job_free(ctx);
                return;
...
static void mln_lang_stack_handler_elemlist(mln_lang_ctx_t *ctx)
{
    ...
    if (node->step == 0) {
        ...
        if (mln_lang_var_setValue_string_ref(ctx, val, ctx->ret_var) < 0) {
            __mln_lang_errmsg(ctx, "No memory.");
            mln_lang_job_free(ctx);
            return;
        }
...
static mln_lang_var_t *mln_lang_msg_func_new_process(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_lang_msg_new(ctx, var->val->data.s) < 0) {
        if ((var = mln_lang_var_createTmpFalse(ctx, NULL)) == NULL) {
            __mln_lang_errmsg(ctx, "No memory.");
            return NULL;
        }
    } else {
        if ((var = mln_lang_var_createTmpTrue(ctx, NULL)) == NULL) {
            __mln_lang_errmsg(ctx, "No memory.");
            return NULL;
        }
    }
...
static mln_lang_var_t *mln_lang_msg_func_recv_process(mln_lang_ctx_t *ctx)
{
    ...
    if (lm->script_read) {
        ...
        if ((var = __mln_lang_var_createTmpNil(ctx, NULL)) == NULL) {
            __mln_lang_errmsg(ctx, "No memory.");
            return NULL;
        }
    } else {
