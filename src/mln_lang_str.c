...
mln_lang_str_assign(mln_lang_ctx_t *ctx, mln_lang_var_t **ret, mln_lang_var_t *op1, mln_lang_var_t *op2)
{
    if (mln_lang_var_setValue_string_ref(ctx, op1, op2) < 0) {
...
mln_lang_str_equal(mln_lang_ctx_t *ctx, mln_lang_var_t **ret, mln_lang_var_t *op1, mln_lang_var_t *op2)
{
    ...
    rv = mln_string_strcmp(mln_lang_var_getVal(op1)->data.s, tmp);
    mln_string_free(tmp);
    if (rv) {
        if ((*ret = mln_lang_var_createTmpFalse(ctx, NULL)) == NULL) {
            mln_lang_errmsg(ctx, "No memory.");
...
mln_lang_str_index(mln_lang_ctx_t *ctx, mln_lang_var_t **ret, mln_lang_var_t *op1, mln_lang_var_t *op2)
{
    mln_s64_t offset;
    mln_string_t c;
    mln_string_t *s = mln_lang_var_getVal(op1)->data.s;
    if (mln_lang_var_getValType(op2) != M_LANG_VAL_TYPE_INT) {
        mln_lang_errmsg(ctx, "Offset must be an integer.");
        return -1;
    }
    offset = mln_lang_var_getVal(op2)->data.i;
    if (offset < 0) {
