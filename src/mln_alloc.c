...
static inline mln_alloc_shm_t *mln_alloc_shm_new(mln_alloc_t *pool, mln_size_t size, int is_large)
{
    ...
    if (tmp == NULL && (mln_u8ptr_t)(pool->mem + pool->capacity) - p < size) {
        return NULL;
    }
...
mln_alloc_t *mln_alloc_shm_init(mln_size_t capacity, void *locker, mln_alloc_shm_lock_cb_t lock, mln_alloc_shm_lock_cb_t unlock)
{
    ...
    if (capacity < M_ALLOC_SHM_DEFAULT_SIZE+1024 || locker == NULL || lock == NULL || unlock == NULL) {
        return NULL;
    }
    ...
    if (fd < 0) return NULL;
    pool = (mln_alloc_t *)mmap(NULL, capacity, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
#else
    pool = (mln_alloc_t *)mmap(NULL, capacity, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANON, -1, 0);
#endif
...
void *mln_alloc_m(mln_alloc_t *pool, mln_size_t size)
{
    ...
    if (am == NULL) {
        ...
        if (pool->capacity) {
            if (size + pool->in_used > pool->capacity)
                return NULL;
    ...
    if (am->free_head == NULL) {
        ...
        if (pool->capacity) {
            if (alloc_size + pool->in_used > pool->capacity) {
                goto oom;
            }

            pool->in_used += alloc_size;
        }
        ...
        if (pool->parent != NULL) {
            ...
            ptr = (mln_u8ptr_t)mln_alloc_m(pool->parent, alloc_size);
            if (mln_alloc_is_shm(pool->parent)) {
                (void)pool->parent->unlock(pool->parent->locker);
            }
        } else {
            ptr = (mln_u8ptr_t)malloc(alloc_size);
        }
        if (ptr == NULL) {
oom:
...
static inline mln_u8_t mln_alloc_shm_count_bits(mln_u8_t n) {
    n = n - ((n >> 1) & 0x55);
    n = (n & 0x33) + ((n >> 2) & 0x33);
    n = (n + (n >> 4)) & 0x0F;
    return n;
...
{
    if (pool->mem == NULL) {
        return pool->capacity? (pool->capacity - pool->in_used): M_ALLOC_INFINITE_SIZE;
    }

    mln_alloc_shm_t *as = pool->shm_head;
    mln_u8ptr_t p, pend;
    mln_size_t sum = 0;
    ...
    for (; as != NULL; as = as->next) {
        p = as->bitmap;
        for (pend = p + M_ALLOC_SHM_BITMAP_LEN; p < pend; ++p) {
            sum += (mln_size_t)mln_alloc_shm_count_bits(*p);
        }
    }
    sum = (((mln_size_t)M_ALLOC_SHM_BITMAP_LEN) << 3) - sum;
    return sum * M_ALLOC_SHM_BIT_SIZE;
}
