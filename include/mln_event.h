...
struct mln_event_s {
    ...
    /*
     * Indicating dispatch() to return when
     * master process fork a child process.
     * If a child process is forked,
     * dispatch() should be break out in child process.
     * and the sockfd handler in child process
     * should set is_child_proc = 1;
     */
    mln_u32_t                is_child_proc:1;
    mln_u32_t                is_break:1;
    mln_u32_t                in_main_thread:30;
    int                      rd_fd;
    int                      wr_fd;
#if defined(MLN_EPOLL)
    int                      epollfd;
...
mln_event_set_fd(mln_event_t *event, \
                 ...
                 mln_u32_t flag, \
                 void *data, \
                 void (*fd_handler)(mln_event_t *, int, void *)) __NONNULL1(1);
extern int
mln_event_set_timer(mln_event_t *event, \
                    mln_u32_t msec, \
                    void *data, \
                    void (*tm_handler)(mln_event_t *, void *)) __NONNULL1(1);
extern int
mln_event_set_signal(mln_event_t *event, \
                     mln_u32_t flag, \
                     int signo, \
                     void *data, \
                     void (*sg_handler)(mln_event_t *, int, void *)) __NONNULL1(1);
